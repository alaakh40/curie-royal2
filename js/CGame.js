// ========== Supabase functions config (ضع التوكن هنا) ==========
const SUPABASE_URL = "https://pmdjkalkmsrwyoqeybeu.supabase.co"; // ضع رابط مشروعك هنا
// ضع هنا توكن الدخول لللاعب (تحصل عليه من نظام الـ login بعد تسجيل دخول اللاعب)
// **لا تضع توكن حقيقي هنا في مكان عام** — هذا مكان للتجربة المحلية.
const ACCESS_TOKEN = "PUT_PLAYER_ACCESS_TOKEN_HERE";

// جلب الرصيد من الـ edge function get-player-balance
async function getBalance() {
  try {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/get-player-balance`, {
      method: "GET",
      headers: {
...
// تحديث الرصيد عبر edge function update-player-balance
// type: 'deduct' | 'add'
async function updateBalance(amount, type, gameId, description) {
  try {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/update-player-balance`, {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${ACCESS_TOKEN}`,
        "Content-Type": "application/json"
      },
      body: JSON.stringify({ amount, type, gameId: gameId || "slot", description: description || "" })
    });
    const data = await res.json();
    return data;
  } catch (err) {
...
function CGame(){
    ...
    this._init = function(){
        ...
        _iMoney = 0; // مؤقت قبل ما تجي بيانات من السيرفر

        // جلب الرصيد من السيرفر مباشرة بعد تهيئة اللعبة
        getBalance().then(data => {
          if (data && data.balance !== undefined) {
            _iMoney = Number(data.balance);
            try {
              // لو الواجهة جاهزة حدث الرصيد
              if (_oInterface && typeof _oInterface.refreshMoney === "function") {
                _oInterface.refreshMoney(_iMoney);
              }
            } catch (e) {
              // لا تفشل إذا الواجهة مش جاهزة بعد
            }
          } else {
            console.warn("getBalance returned no data or error", data);
          }
        }).catch(err => {
    ...
    // ====== onSpin needs to call updateBalance => اجعلنا async ======
    this.onSpin = async function(){
        _iCurMinLoops = MIN_REEL_LOOPS;
        ...
        // اضفنا هنا طلب للتحديث (خصم المبلغ) — نستخدم edge function
        try {
          const res = await updateBalance(Number(_iTotBet), "deduct", "slot", "Spin bet");
          if (res && res.success && res.newBalance !== undefined) {
            _iMoney = Number(res.newBalance);
            if (_oInterface && typeof _oInterface.refreshMoney === "function") {
              _oInterface.refreshMoney(_iMoney);
            }
          } else {
            // لو ما نجح الخصم، نرجع اللي صار ونعرض رسالة
            console.warn("updateBalance(deduct) did not return success:", res);
            // ممكن تعرض نافذة خطأ او ترجّع الزر
            s_oMsgBox.show("Transaction failed", "Could not place bet. Please check your balance.");
            _oInterface.enableGuiButtons();
            return;
          }
        } catch (err) {
          console.error("Error calling updateBalance onSpin:", err);
          s_oMsgBox.show("Error", "Network or server error while placing bet.");
          _oInterface.enableGuiButtons();
          return;
        }
    ...
    // ====== onSpinReceived can credit wins => اجعلنا async ======
    this.onSpinReceived = async function( oData){
        _iNumAds++;	
        ...
        if ( oData.res === true ){	
            ...
            // اذا اللاعب ربح، استدعي updateBalance مع type 'add'
            if (fWinAmount && fWinAmount > 0) {
              try {
                const res = await updateBalance(Number(fWinAmount), "add", "slot", "Spin win");
                if (res && res.success && res.newBalance !== undefined) {
                  _iMoney = Number(res.newBalance);
                  if (_oInterface && typeof _oInterface.refreshMoney === "function") {
                    _oInterface.refreshMoney(_iMoney);
                  ...
                  console.warn("updateBalance(add) did not return success:", res);
                }
              } catch (err) {
                console.error("Error calling updateBalance(add):", err);
              }
            }
